...
class unpaired_dataset(data.Dataset):
    def __init__(self, args, phase='train'):
        self.args = args
        self.images_source = glob.glob(os.path.join("/media/ml/Data Disk/CapsNetwork/Unsupervised/DATASET/Conventional/Train/Cropped_HR", '*.jpg'))
        ...
        self.images_target = glob.glob(os.path.join("/media/ml/Data Disk/CapsNetwork/Unsupervised/DATASET/Train/HR", '*.*'))
        if phase == 'train':
            self.images_source = self.images_source[:-33]
        else:
            self.images_source = glob.glob(os.path.join("/media/ml/Data Disk/CapsNetwork/Unsupervised/DATASET/Conventional/Train/Cropped_HR", '*.jpg'))#self.images_source[:]
            self.images_target = glob.glob(os.path.join("/media/ml/Data Disk/CapsNetwork/Unsupervised/DATASET/Train/HR", '*.*'))
