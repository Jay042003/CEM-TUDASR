...
class Saver():
    ...
    def write_img_SR(self, ep, sr, filename):
        ...
        img_filename = os.path.join(result_savepath, filename[0][:-3]+'png')
    ...
    def write_best_sr(self, ep, total_it, model,is_best=True):
        print('saving the best sr model')
        model.model_save('%s/best_sr_model.pth' % (self.model_dir), ep, total_it)
    
    def write_best_down(self, ep, total_it, model):
        print('saving the best down model')
        model.model_save('%s/best_down_model.pth' % (self.model_dir), ep, total_it)
