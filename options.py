...
class Options():
    def __init__(self):
        ...
        self.parser.add_argument('--source', type=str, default='/media/admin1/DL/CapsNetwork/Unsupervised/DATASET/Conventional/Train/Cropped_HR/', help='Source type')
        self.parser.add_argument('--target', type=str, default='/media/admin1/DL/CapsNetwork/Unsupervised/DATASET/Train/HR/', help='target type')
        ...
        ## data loader related
        self.parser.add_argument('--train_dataroot', type=str, default='/media/admin1/DL/CapsNetwork/Unsupervised/DATASET/Conventional/Train/', help='path of train data')
        self.parser.add_argument('--test_dataroot', type=str, default='/media/admin1/DL/CapsNetwork/Unsupervised/DATASET/Train/', help='path of test data')
        self.parser.add_argument('--phase', type=str, default='train', help='phase for dataloading')
        self.parser.add_argument('--batch_size', type=int, default=10, help='batch size')
        self.parser.add_argument('--patch_size_down', type=int, default=192, help='cropped image size for learning downsampling')
        ...
        ## ouptput related
        self.parser.add_argument('--name', type=str, default=None, help='folder name to save outputs')
        self.parser.add_argument('--experiment_dir', type=str, default='./experiment', help='path for saving result images and models')
        self.parser.add_argument('--result_dir', type=str, default='./experiment', help='path for saving result images and models')
        ...
        ## training related
        # common
        self.parser.add_argument('--gpu', type=str, default='cuda:3', help='gpu ids: e.g. 0  0,1,2, 0,2')
        self.parser.add_argument('--scale', type=int, choices=(2, 4, 8), default=8, help='scale to SR, only support [2, 4]')
        # learning downsampling
        ...
        self.parser.add_argument('--epochs_down', type=int, default=25, help='number of epochs for training downsampling')
        self.parser.add_argument('--lr_down', type=float, default=0.00001, help='learning rate for learning downsampling')
        ...
        self.parser.add_argument('--epochs_sr_start', type=int, default=51, help='start epochs for training SR')
        self.parser.add_argument('--epochs_sr_end', type=int, default=25, help='end epochs for training SR')
        self.parser.add_argument('--lr_sr', type=float, default=0.5*0.00001, help='learning rate for training SR')
        self.parser.add_argument('--adv_w', type=float, default=1, help='weight for adversarial loss in esrgan training')
        self.parser.add_argument('--per_w', type=float, default=1.0, help='weight for adversarial loss in esrgan training')
        self.parser.add_argument('--con_w', type=float, default=1.0, help='weight for adversarial loss in esrgan training')
        self.parser.add_argument('--wav_w', type=float, default=0, help='weight for wavelet loss in esrgan training')
        self.parser.add_argument('--percep_w', type=float, default=0, help='weight for perceptual loss in esrgan training')
        self.parser.add_argument('--edge_loss_w', type=float, default=1e-2, help='weight for btv loss')
        self.parser.add_argument('--gv_w', type=float, default=0, help='weight for gradient variance loss in esrgan training')
        ...
        self.parser.add_argument('--sr_model', type=str, choices=('edsr', 'rrdb', 'endosr', 'gan'), default='edsr', help='choose model to SR')
        self.parser.add_argument('--training_type', type=str, default='edsr', choices=('edsr', 'esrgan', 'endosr', 'gan'), help='choose training type of SR')
...
class TestOptions():
    def __init__(self):
        ...
        # validation set for SR, only used in joint training
        self.parser.add_argument('--test_hr', type=str, default=None, help='HR images for validating')
        self.parser.add_argument('--test_lr', type=str, default='Capsule_Data/TestSet/Capsule_dataset02', help='LR images for validating')
        ## data loader related
        # self.parser.add_argument('--train_dataroot', type=str, default='../datasets/', help='path of train data')
        self.parser.add_argument('--test_dataroot', type=str, default='', help='path of test data')
        self.parser.add_argument('--phase', type=str, default='test', help='phase for dataloading')
        self.parser.add_argument('--test_mode', type=str, default='sr', help='sr, down')
        self.parser.add_argument('--batch_size', type=int, default=1, help='batch size')
        self.parser.add_argument('--patch_size_down', type=int, default=512, help='cropped image size for learning downsampling, HR patch size')
        self.parser.add_argument('--nThreads', type=int, default=8, help='# of threads for data loader')
        self.parser.add_argument('--crop', type=int, default=336, help='specified if not use bin')
        self.parser.add_argument('--flip', action='store_true', help='specified if flip')
        self.parser.add_argument('--rot', action='store_true', help='specified if rotate')
        ## ouptput related
        ...
        self.parser.add_argument('--scale', type=int, choices=(2, 4, 8), default=2, help='scale to SR, only support [2, 4]')
        # testing downsampler
        self.parser.add_argument('--resume_down', type=str, default=None, help='load training states for resume the downsampling learning')
        self.parser.add_argument('--dis_norm', type=str, default='Instance', help='normalization layer in discriminator [None, Batch, Instance, Layer]')
        self.parser.add_argument('--gen_norm', type=str, default='Instance', help='normalization layer in generator [None, Batch, Instance, Layer]')
        ...
        self.parser.add_argument('--resume_sr', type=str, default=None, help='load training states for resume the downsampling learning')
        self.parser.add_argument('--sr_model', type=str, choices=('edsr', 'rrdb', 'endosr', 'gan'), default='edsr', help='choose model to SR')
        self.parser.add_argument('--training_type', type=str, default='edsr', choices=('edsr', 'rrdb', 'gan', 'endosr'), help='choose training type of SR')
        self.parser.add_argument('--precision', type=str, choices=('single', 'half'), default='singe', help='precision for forwarding SR')
        self.parser.add_argument('--realsr', action='store_true', help='just make SR image without calculating PSNR')
        self.parser.add_argument('--chop', action='store_true', help='enable memory-efficient forward')
        self.parser.add_argument('--test_range', type=str, default='1-5000', help='test data range')
        ## experimnet related
        self.parser.add_argument('--save_log', action='store_true', help='enable saving log option')
        self.parser.add_argument('--save_results', action='store_true', help='enable saving intermediate image option')
        self.parser.add_argument('--edsr_format', type=bool, default=False, help='save image as EDSR format')
