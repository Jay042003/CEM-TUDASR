...
from utils.utility import log_writer, plot_loss_down, plot_psnr, timer, calc_psnr, quantize, _normalize
"""
scale 8: --name EndoSR_withoutWL/endosr_x2 --adv_w 0.01 --wav_w 0 --scale 2 --batch_size 1 --patch_size_down 512 --decay_batch_size_sr 80000 --decay_batch_size_down 60000 --epochs_sr_start 51 --gpu cuda:3 --sr_model endosr --training_type endosr --joint --save_results --save_log
scale 4: --name EndoSR_withoutWL/endosr_x4 --adv_w 0.01 --wav_w 0 --scale 4 --batch_size 10 --patch_size_down 256 --decay_batch_size_sr 400000 --decay_batch_size_down 50000 --epochs_sr_start 51 --gpu cuda:2 --sr_model endosr --training_type endosr --joint --save_results --save_log
"""
## parse options
parser = Options()
args = parser.parse()
...
train_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.nThreads)
dataset = unpaired_dataset(args, phase='test')
test_loader_down = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=args.nThreads)
...
for ep in range(ep0, max_epochs):
    ...
    if args.joint and ((ep + 1) in range(args.epochs_sr_start, args.epochs_sr_end + 1)):
        ...
        dis_txt1 = ' dis lr %.06f' % (SRM.dis_opt.param_groups[0]['lr'])
        sr_txt2 = '    SR loss    |'
        dis_txt = '  Dis loss   |'

    training_log.write('\n[ epoch %03d/%03d ]   G lr %.06f D lr %.06f%s%s'
                       % (ep + 1, max_epochs, ADM.gen_opt.param_groups[0]['lr'], ADM.dis_opt.param_groups[0]['lr'], sr_txt1,dis_txt1))
    print_txt = '|    Progress   |    Dis    |    Gen    |    data    |%s %s' % (sr_txt2, dis_txt)
    ...
    for it, (img_s, img_t, _) in enumerate(train_loader):
        ...
        sr_loss_txt, sr_timer_txt, dis_loss_txt = '', '', ''
        if args.joint and ((ep + 1) in range(args.epochs_sr_start, args.epochs_sr_end + 1)):
            train_timer_sr.tic()
            ...
            if args.noise:
                n = args.noise_std * torch.Tensor(np.random.normal(size=img_lr.shape)).cuda(args.gpu) / 255.0
            ...
            SRM.update_img(img_lr, img_hr, img_t)
            SRM.generate_HR()
            SRM.update_G()

             
            loss_sr_item += SRM.gen_loss
            loss_dis_gen_item += SRM.dis_loss
            train_timer_sr.hold()
        ...
        if (it + 1) % (len(train_loader) // 10) == 0:
            if args.joint and ((ep + 1) in range(args.epochs_sr_start, args.epochs_sr_end + 1)):
                ...
                loss_dis_gen_item_avg = loss_dis_gen_item / cnt
                # print(loss_dis_gen_item_avg)
                current_sr_loss += loss_sr_item_avg
                sr_loss_txt = '  %0.5e  |' % (loss_sr_item_avg)
                dis_loss_txt = ' %0.5e |' % (loss_dis_gen_item_avg)
            ...
            current_adm_loss += loss_dis_item_avg + loss_gen_item_avg + loss_data_item_avg
            training_log.write('|   %04d/%04d   |  %.05f  |  %.05f  |  %.06f  |%s %s %.01f+%.01fs %s'
                               % ((it + 1), len(train_loader), loss_dis_item_avg, loss_gen_item_avg,
                                  loss_data_item_avg, sr_loss_txt, dis_loss_txt,
    ...
    if current_sr_loss < best_sr_loss and ep>=args.epochs_sr_start:
        training_log.write('Saving best sr model..')
        best_sr_loss = current_sr_loss
        saver.write_best_sr(-1, total_it + 1, SRM)
    if current_adm_loss<best_adm_loss:
        training_log.write('Saving best down model..')
        best_adm_loss = current_adm_loss
        saver.write_best_down(-1, total_it + 1, ADM)
